...
class ImageGenerationService
{
        ...
        if (config('app.puppeteer_mode') === 'sidecar-aws') {
            try {
                $browsershot = BrowsershotLambda::html($markup)
                    ->windowSize($imageSettings['width'], $imageSettings['height']);
            ...
            try {
                $browsershot = Browsershot::html($markup)
                    // ->setNodeBinary('/Users/benjamin.nussbaum/.nvm/versions/node/v20.19.2/bin/node')
    ...
    private static function getImageSettings(Device $device): array
    {
        ...
        return [
            ...
            'colors' => 2, // Default for legacy devices
            'bit_depth' => 1, // Default for legacy devices
            'scale_factor' => 1.0,
